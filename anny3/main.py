...
class LoginScreen(Screen):
    ...
    def offline_login(self, nickname):
        try:
            ...
            app.generic_popup(
                "Sorry for this bug. Reload the app and try again. It seems to work after 3-4 tries",
                              paragraph=True)
...
class GameLobbyScreen(Screen):
    ...
    def notify(self, event):
        ...
        if isinstance(event, e.NewPlayerLineupEvent):
            ...
            app.root.switch_to(GameScreen(event.team_scores, self.is_observer))
...
class GameScreen(Screen):
    ...
    def __init__(self, team_scores, is_observer, *args, **kwargs):
        ...
        self.word_guessed_button = Button(text="Next Player")
        self.word_guessed_button.bind(on_release = self.post_end_turn)
        self.skip_word_button = Button(text = "Skip Word")
        self.skip_word_button.bind(on_release = self.skip_word)
        self.start_time = 0
        self.turn_time = 0
    ...
    def skip_word(self, *args):

        #print "skipping word"
        if not self.is_observer:
            self.end_turn_setup()
        app.lobby.callRemote("notify", e.SkipWordEventRequest())

    def notify(self, event):
        # print event.name, self.is_my_turn
        #TODO: REFACTOR TO DICT
        if isinstance(event, e.StartRoundEvent):
            self.bottom_buttons.clear_widgets()
            if self.is_observer:
                self.bottom_buttons.add_widget(self.skip_word_button)
            ...
            self.team_scores_label_dict[event.team_id].score += 1
        elif isinstance(event, e.SkipWordEventRequest) and self.is_my_turn:
            self.end_turn_setup()#ADDED
            app.lobby.callRemote("notify", e.SkipWordEvent(self.time_remaining()))
...
class ManageWordListsScreen(Screen):
    ...
    def build_screen(self):
        ...
        if word_list_names == []:
            self.scroll_view.add_widget(Label(text=""))
        else:
